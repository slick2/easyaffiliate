...
//// Function for where
 ...
 // Get Categories
  ...
  if($pdo)
  {
  $query5 = "SELECT count(intID) FROM tblarticles WHERE intCategory= ? AND intStatus = 1";
  $bind = array($result[$i]['intID']);
  $qry2Result = select_pdo($query5,$bind);
  }else{
  $query5 = "SELECT count(intID) FROM tblarticles WHERE intCategory=" . $result[$i]['intID']." AND intStatus = 1";
  $qry5Result = $d->fetch($query5); // expires in one day
  }
  
  $numArticles = $sum + $qry5Result[0]['count(intID)'];
  }
  else
  {
  if($pdo)
  {
  $query4 = "SELECT count(intID) FROM tblarticles WHERE intCategory= ? AND intStatus = 1";
  $bind = array($result[$i]['intID']);
  $qry4Result = select_pdo($query4,$bind);
  }else{
  $query4="SELECT count(intID) FROM tblarticles WHERE intCategory=".$result[$i]['intID']." AND intStatus = 1";
  $qry4Result = $d->fetch($query4); // expires in one day
  }
  $numArticles = $qry4Result[0]['count(intID)'];
