...
$result = insert_pdo($query,$bind);
	...
	header("location:index.php?filename=bannedauthor&pageno=".$pageno);
	die();
}
...
if(isset($_REQUEST['a']) && trim($_REQUEST['a'])==Yes)
{
    ...
    if($pdo)
{
$query = "UPDATE tblauthor SET txtBAN = ?, intStatus = ? where intId = ?";
$bind = array("Yes",0,$authorId);
$result = update_pdo($query,$bind);
}else{

		$sql_ban = "UPDATE tblauthor SET txtBAN = 'Yes', intStatus = 0 where intId ='".safeEscapeString($authorId)."'";
    $del = $d->exec($sql_ban);
}
...
{
$query = "UPDATE tblauthor SET txtBAN = ?, intStatus = ? where intId = ?";
$bind = array("No",1,$authorId);
$result = update_pdo($query,$bind);
}else{
		$sql_ban = "UPDATE tblauthor SET txtBAN = 'No', intStatus = 1 where intId ='".safeEscapeString($authorId)."'";
...
// End of BAN operation BAN operation
...
$result = delete_pdo($query,$bind);
		...
		$sql_del = "Delete from tblauthor where intId ='".safeEscapeString($authorId)."'";
		$del = $d->exec($sql_del);
    }
...
$result = delete_pdo($query,$bind);
		...
		$sql_delart = "Delete from tblarticles where intAuthorId ='".safeEscapeString($authorId)."'";
...
$result = select_pdo($query,$bind);
		...
		$sql = "select * from tblauthor where intId ='".safeEscapeString($authorId)."'";
		$result = $d->fetch($sql);
}		
...
$result = update_pdo($query,$bind);
		...
		$update = $d->exec("update tblauthor set intStatus = 1 where intId='".safeEscapeString($id)."'");
}
	}
}
...
$result = update_pdo($query,$bind);
		...
		$update = $d->exec("update tblauthor set intStatus = 0 where intId='".safeEscapeString($id)."'");
}
	}
}
?>
...
<table border="0" align="center" cellpadding="1" cellspacing="0" width="725">
	  <tr>
		...
		<table border="0" align="center" cellpadding="1" cellspacing="0">
			<tr align="center" class="line_top">
				<td width="75%">Banned Authors</td>
				<td width="100%" align="right"><a class="link" href="index.php?filename=bannedauthor&script=addauthor"><font color='#53A8BF'>Add Author</font></a></td>
			</tr>
		</table>
		</td>
	  </tr>
	...
	<?
		...
		**************************************/
		...
		$per_page_keywords = "tblauthor.intCountry = tblcountry.intId AND intStatus = 0 AND txtBAN = 'Yes'";
		$per_page_sorts="";
...
$query = "SELECT * FROM tblcountry, tblauthor WHERE tblauthor.intCountry = tblcountry.intId AND intStatus = ? AND txtBAN = ? ORDER BY dtRegisteredDate DESC Limit ?,?";
$bind = array(0,"Yes",$limit,$row_per_page);
	...
	$i=0;
	 ...
	 { 
		 $i=$i+1;
		  ?>
			<tr class="<?php echo ($i%2==0)?"Hrnormal":"Hralter"; ?>" onMouseOver="this.className='Hrhover';"  onMouseOut="this.className='<?php echo ($i%2==0)?"Hrnormal":"Hralter"; ?>';">        
				...
				{
					$intId = $row['intId'];
					echo "<td><a class='link' href='index.php?filename=bannedauthor&pageno=".$pageno."&s=0&authorid=$intId'>Unapproved</a></td>";		
				}
				if($row['intStatus']==1)
				{
					$intId = $row['intId'];
					echo "<td><a class='link' href='index.php?filename=bannedauthor&pageno=".$pageno."&s=1&authorid=$intId'>APPROVED</a></td>";
				}
				?>
        <td align="center"><?php
        $photo = $row['authPhoto'];
        ...
        <?php 
				...
				{
					$intId = $row['intId'];
					echo "<td align='center'><a class='link' href='index.php?filename=bannedauthor&pageno=".$pageno."&a=Yes&authorid=$intId' onClick='return confirm('Are you sure you wish to ban this author?');'>No</a></td>";		
				}
				if($row['txtBAN']=='Yes')
				{
					$intId = $row['intId'];
					echo "<td align='center'><a class='link' href='index.php?filename=bannedauthor&pageno=".$pageno."&a=No&authorid=$intId' onClick='return confirm('Are you sure you wish to unban this author?');'>Yes</a></td>";
				}
				?>
