...
$author = select_pdo($query);
		  ...
		  foreach($author as $row) {
		  ?>
		  <option value="<?=$row['intId'];?>" <?php if($row['intId']==$AuthorId){echo "selected";}else{echo "";}?>><?=stripslashes($row['varFirstName'])." ".stripslashes($row['varlastName']);?></option>
		  <?php }
		  ?>
          </select></td>
	  </tr>
...
$author = select_pdo($query);
		  ...
		  foreach($author as $row) {
				...
				?>
		<option value="<?=$row['intID'];?>" <?php if($row['intID']==$Category){echo "selected";}else{echo "";}?>><?=$row['varCategory'];?></option>
		<?php } ?>
