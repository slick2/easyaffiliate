    ...
    if (isset($_REQUEST['a']) && trim($_REQUEST['a']) == No) {
        if (isset($_REQUEST['authorid']) && trim($_REQUEST['authorid'] != "")) {
            ...
            if ($pdo) {
                ...
                $sql_ban = "UPDATE tblauthor SET txtBAN = 'No' where intId ='" . safeEscapeString($authorId) . "'";
        ...
        <table border="0" align="center" cellpadding="1" cellspacing="0" width="725">
                ...
                <td height="20">
                    <table border="0" align="center" cellpadding="1" cellspacing="0" width="725">
                        <tr align="center" class="line_top">
                            <td width="75%">Approved Authors</td>
                            <td width="100%" style='float:right;'><a class="link" href="index.php?filename=author&script=addauthor"><font color='#53A8BF'>Add Author</font></a></td>
            ...
            <td >
                <table  border="0" align="center" cellpadding="2" cellspacing="2" class="greyborder" width="725">
                    ...
                    $per_page_keywords = "tblauthor.intCountry = tblcountry.intId AND intStatus = 1";
                    $per_page_sorts = "";
                    ...
                    if ($sql) {
                        ...
                        foreach ($sql as $row) {
                            ...
                            <tr class="<?php echo ($i % 2 == 0) ? "Hrnormal" : "Hralter"; ?>" onMouseOver="this.className = 'Hrhover';"  onMouseOut="this.className = '<?php echo ($i % 2 == 0) ? "Hrnormal" : "Hralter"; ?>';">
                                ...
                                if ($row['intStatus'] == 0) {
                                    $intId = $row['intId'];
                                    echo "<td><a class='link' href='index.php?filename=author&pageno=" . $pageno . "&s=0&authorid=$intId'>Unapproved</a></td>";
                                }
                                if ($row['intStatus'] == 1) {
                                    $intId = $row['intId'];
                                    echo "<td><a class='link' href='index.php?filename=author&pageno=" . $pageno . "&s=1&authorid=$intId'>APPROVED</a></td>";
                                }
                                ?>
                                <td align="center"><?php
                                    $photo = $row['authPhoto'];
                                ...
                                if ($row['txtBAN'] == 'No') {
                                    $intId = $row['intId'];
                                    echo "<td align='center'><a class='link' href='index.php?filename=author&pageno=" . $pageno . "&a=Yes&authorid=$intId' onClick='return confirm('Are you sure you wish to ban this author?');'>No</a></td>";
                                }
                                if ($row['txtBAN'] == 'Yes') {
                                    $intId = $row['intId'];
                                    echo "<td align='center'><a class='link' href='index.php?filename=author&pageno=" . $pageno . "&a=No&authorid=$intId' onClick='return confirm('Are you sure you wish to unban this author?');'>Yes</a></td>";
                                }
                                ?>
