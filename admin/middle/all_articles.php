...
$author = select_pdo($query);
		  ...
		  ?>
		  <option value="<?=$row['intId'];?>" <?php if($row['intId']==$AuthorId){echo "selected";}else{echo "";}?>><?=stripString($row['varFirstName'])." ".stripString($row['varlastName']);?></option>
		  <?php }
		  ?>
          </select></td>
	  </tr>
...
$author = select_pdo($query);
		  ...
		  foreach($author as $row) {
				...
				?>
		<option value="<?=$row['intID'];?>" <?php if($row['intID']==$Category){echo "selected";}else{echo "";}?>><?=stripString($row['varCategory']);?></option>
		<?php 
    } 
    ?>
