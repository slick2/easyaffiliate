...
<?php
  $var = safeEscapeString($_POST['art_text']) ;
  $trimmed = trim($var);
  $limit=10; 
...
if (!isset($var))
  {
  echo "<p>We dont seem to have a search parameter!</p><br><br>";
  exit;
  }
...
{
$var = $_POST['art_text'];
$query = "select intAuthorId,varArticleTitle,textSummary from tblarticles WHERE textArticleText like ?";
$bind = array('%'.$var.'%');
$connection = select_pdo($query,$bind);
}else{
$var = safeEscapeString($_POST['art_text']);
$connection = $d->fetch("select * from tblarticles WHERE textArticleText like '%$var%'");
}
...
$a = $s + ($limit) ;
  ...
  echo "<br><br><div align='center'><a href='index.php?filename=dupe_form'>Back To Search Form</a>";
} 
